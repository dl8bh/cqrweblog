...
class Adif
{
    ...
    */
    ...
    {
        /* 
            This function returns a list of QSOs from a string representation of an adif-file
        */

        // Explode the string first
        $exploded = (explode("<", $adif_file_string));

        // In this array the resulting 
        $return_array = array();
        // Switch to QSO-Reading mode, when the actual records begin
        $header = true;
        $adif_array = array();
            ...
            if ($header == false) {
                if (preg_match('/(?P<FIELDNAME>\V+):(?P<LENGTH>\d+)>(?P<RAWVALUE>\V+)/', $item, $matches)) {
                    ...
                    */
                    ...
                    /* TODO: STATION-CALLSIGN, OPERATOR could be both CALLSIGN */
                    // Add the QSO to the array of QSOs
                    array_push($return_array, new Qso($qso_array));
                    $qso_array = array();
                }
            }
        }
